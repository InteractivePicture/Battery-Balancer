...
function Animate(to,batt) {

	bat = batt;

	animate = true;
	// richting zit in de tellers
	if (bat == 3) {
	...
	stapgrootte1 = Math.abs(hoogte1-eindwaarde)/aantalStappen;
	teller1 = hoogte1;

	stapgrootte2 = Math.abs(hoogte1-eindwaarde)/aantalStappen;
	teller2 = hoogte2;
...
function Onbalans() {
	...
	if ( Math.abs(midpoint) < 0.2 ) {
		...
		onbalans = setTimeout(Onbalans,100);
		ToonStatus();
	} else {
		
		stap = 0; $("#overlay").hide(); balancer = true; Start();
	}
	
}

var list = "70,2#300,1#400,1#100,2#70,1#300,2".split("#");
var listlengte = list.length;
var listpos = 0;
...
function DoBalance() {
	...
	if ( stap < aantalStappen ) {
		...
		dobalance = setTimeout(DoBalance,40); // 40
		...
		if (animate) {
			...
			// 
			nrs = list[listpos].split(",");
			listpos++;
			if (listlengte == listpos) { listpos = 0; } 
 
				
			vertraag = setInterval(function () {

				clearInterval(vertraag); 
...
$( "#progplay" ).mouseup(function() {
	if ( Math.abs(spanning1.toFixed(2)) == 13.50 && Math.abs(spanning2.toFixed(2)) == 13.50 ) {  Animate(90,1); } else { location.reload(); }
});
